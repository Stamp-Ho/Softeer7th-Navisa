...
const NoChatView = ({ isFileReady }: NoChatViewParams) => {
  ...
  const isAgent = userType === "VALID_AGENT" || userType === "INVALID_AGENT";

  return (
    <div className="flex flex-col mt-12 h-[824px]">
      ...
      <div className="flex flex-col gap-4 justify-center items-center h-full">
        ...
        <Link
          to={
            isFileReady
              ? `/search/${isAgent ? "foreigner" : "agent"}`
              : `/onboard/${isAgent ? "agent" : "foreigner"}`
          }
          className="flex flex-row items-center pl-5 title-m-semibold text-violet-500 cursor-pointer"
        >
          {isFileReady
            ? isAgent
              ? t("chat.searchForeigner")
              : t("chat.searchAgent")
            : t("chat.registerRequirement")}
