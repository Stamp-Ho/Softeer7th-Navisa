...
const SearchForeignerCard = ({
  ...
  return (
    <div
      className={`flex flex-col py-7 px-6 w-92 h-fit rounded-2xl ${withDetails ? "shadow bg-white" : " bg-gray-30"}`}
    >
      <Link to={`/profile/foreigner/${foreigner.foreignerId}`}>
        ...
        <h4 className="mt-2 title-m-bold text-text-base">
          {foreigner.nickname}
        </h4>
...
const SkeletonUi = (withDetails = true) => {
  return (
    ...
    >
      ...
      <Tag type="small_fill_gray" className="w-32 mt-2 mb-1" />
        ...
        <div className="flex flex-col gap-3">
          <div className="flex flex-col gap-2">
            <div className="flex flex-row gap-1 items-center w-36">
              ...
              <Tag type="tiny_skeleton" className="w-22" />
            </div>
            <Tag type="small_fill_gray_2" className="rounded-sm w-40" />
          </div>
          {withDetails && (
            <div className="flex flex-col w-36 gap-2">
              <div className="flex flex-row gap-1 items-center">
                <Tag type="tiny_skeleton" />
                <Tag type="tiny_skeleton" className="w-13" />
              </div>
              <Tag type="small_fill_gray" className="w-10" />
            </div>
          )}
        </div>
        <div className="flex flex-col w-36 gap-2">
