export const RPMessage = () => {
  return (
    <svg
      width="56"
      height="56"
      viewBox="0 0 56 56"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g clipPath="url(#clip0_2784_28943)">
        <circle
          cx="27.598"
          cy="28"
          r="28"
          transform="rotate(-45 27.598 28)"
          fill="url(#paint0_linear_2784_28943)"
        />
        <rect
          x="11.2002"
          y="16.8706"
          width="33.599"
          height="23.5193"
          rx="2.79992"
          fill="url(#paint1_linear_2784_28943)"
        />
        <mask
          id="mask0_2784_28943"
          style={{ maskType: "alpha" }}
          maskUnits="userSpaceOnUse"
          x="11"
          y="16"
          width="34"
          height="25"
        >
          <rect
            x="11.2009"
            y="16.8701"
            width="33.599"
            height="23.5193"
            rx="2.79992"
            fill="#787F8B"
          />
        </mask>
        <g mask="url(#mask0_2784_28943)">
          <path
            d="M10.0808 41.0879L22.9604 30.4482"
            stroke="url(#paint2_linear_2784_28943)"
            strokeWidth="0.933306"
          />
          <path
            d="M45.9197 41.0879L33.0401 30.4482"
            stroke="url(#paint3_linear_2784_28943)"
            strokeWidth="0.933306"
          />
          <path
            d="M26.1227 30.3912L10.0808 15.8901L25.8687 34.4278C26.9866 35.7405 29.014 35.7405 30.1319 34.4278L45.9198 15.8901L29.8779 30.3912C28.8117 31.355 27.1889 31.355 26.1227 30.3912Z"
            fill="url(#paint4_linear_2784_28943)"
          />
        </g>
      </g>
      <defs>
        <linearGradient
          id="paint0_linear_2784_28943"
          x1="27.598"
          y1="-1.90735e-06"
          x2="27.598"
          y2="56"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0.5" stopColor="#E7E3FF" />
          <stop offset="1" stopColor="#C5F5FC" />
        </linearGradient>
        <linearGradient
          id="paint1_linear_2784_28943"
          x1="11.8"
          y1="16.8034"
          x2="44.799"
          y2="40.4027"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0.5" stopColor="#B3A8FF" />
          <stop offset="1" stopColor="#71CCEB" />
        </linearGradient>
        <linearGradient
          id="paint2_linear_2784_28943"
          x1="24.6115"
          y1="29.2935"
          x2="11.7896"
          y2="40.0027"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#7E99F1" />
          <stop offset="1" stopColor="#B0A4FF" stopOpacity="0" />
        </linearGradient>
        <linearGradient
          id="paint3_linear_2784_28943"
          x1="31.389"
          y1="29.2935"
          x2="44.2109"
          y2="40.0027"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#7E99F1" />
          <stop offset="1" stopColor="#B0A4FF" stopOpacity="0" />
        </linearGradient>
        <linearGradient
          id="paint4_linear_2784_28943"
          x1="28.0003"
          y1="16.8701"
          x2="28.0003"
          y2="36.9307"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#3E2FA6" />
          <stop offset="1" stopColor="#9FAEFE" />
        </linearGradient>
        <clipPath id="clip0_2784_28943">
          <rect width="56" height="56" rx="28" fill="white" />
        </clipPath>
      </defs>
    </svg>
  );
};

export const RPPeople = () => {
  return (
    <svg
      width="56"
      height="56"
      viewBox="0 0 56 56"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g clipPath="url(#clip0_2784_28975)">
        <circle
          cx="27.598"
          cy="28"
          r="28"
          transform="rotate(-45 27.598 28)"
          fill="url(#paint0_linear_2784_28975)"
        />
        <mask
          id="mask0_2784_28975"
          style={{ maskType: "alpha" }}
          maskUnits="userSpaceOnUse"
          x="26"
          y="24"
          width="26"
          height="19"
        >
          <path
            d="M48.8015 24.3359L51.3015 40.8359L37.8015 42.3359C37.8015 32.3909 33.4161 27.5491 27.4014 28.3867L26.3796 28.5801C26.725 28.4979 27.0657 28.4335 27.4014 28.3867L48.8015 24.3359Z"
            fill="url(#paint1_linear_2784_28975)"
          />
        </mask>
        <g mask="url(#mask0_2784_28975)">
          <path
            d="M27.6121 36.7092C27.6121 31.5012 31.834 27.2793 37.042 27.2793C42.25 27.2793 46.4719 31.5012 46.4719 36.7092V36.8838C46.4719 37.9447 45.6118 38.8047 44.551 38.8047H29.533C28.4721 38.8047 27.6121 37.9447 27.6121 36.8838V36.7092Z"
            fill="url(#paint2_linear_2784_28975)"
          />
        </g>
        <path
          d="M12 39.1231C12 32.7891 17.1348 27.6543 23.4688 27.6543C29.8028 27.6543 34.9376 32.7891 34.9376 39.1231V39.3355C34.9376 40.6258 33.8916 41.6717 32.6014 41.6717H14.3362C13.046 41.6717 12 40.6257 12 39.3355V39.1231Z"
          fill="url(#paint3_linear_2784_28975)"
        />
        <circle cx="23.469" cy="19.053" r="6.05298" fill="#B3A8FF" />
        <circle cx="37.0413" cy="20.2064" r="4.97689" fill="#B3A8FF" />
        <mask
          id="mask1_2784_28975"
          style={{ maskType: "alpha" }}
          maskUnits="userSpaceOnUse"
          x="12"
          y="27"
          width="23"
          height="15"
        >
          <path
            d="M12.0005 39.1231C12.0005 32.7891 17.1352 27.6543 23.4693 27.6543C29.8033 27.6543 34.9381 32.7891 34.9381 39.1231V39.3355C34.9381 40.6258 33.8921 41.6717 32.6019 41.6717H14.3367C13.0465 41.6717 12.0005 40.6257 12.0005 39.3355V39.1231Z"
            fill="url(#paint4_linear_2784_28975)"
          />
        </mask>
        <g mask="url(#mask1_2784_28975)">
          <path
            opacity="0.5"
            d="M28.3013 42.3359C32.7013 35.1359 26.8013 27.8359 22.3013 27.3359H33.3013L37.3013 42.3359H28.3013Z"
            fill="url(#paint5_linear_2784_28975)"
          />
        </g>
        <mask
          id="mask2_2784_28975"
          style={{ maskType: "alpha" }}
          maskUnits="userSpaceOnUse"
          x="17"
          y="13"
          width="13"
          height="13"
        >
          <circle
            cx="23.4695"
            cy="19.053"
            r="6.05298"
            fill="url(#paint6_linear_2784_28975)"
          />
        </mask>
        <g mask="url(#mask2_2784_28975)">
          <path
            opacity="0.5"
            d="M26.3013 18.3359C26.3013 11.8359 20.9679 12.5026 18.8013 12.3359H29.5344V25.3359H21.0932C20.9917 25.3443 20.8941 25.3444 20.8013 25.3359H21.0932C22.9966 25.1797 26.3013 22.1334 26.3013 18.3359Z"
            fill="url(#paint7_linear_2784_28975)"
          />
        </g>
      </g>
      <defs>
        <linearGradient
          id="paint0_linear_2784_28975"
          x1="27.598"
          y1="-1.90735e-06"
          x2="27.598"
          y2="56"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0.5" stopColor="#E7E3FF" />
          <stop offset="1" stopColor="#C5F5FC" />
        </linearGradient>
        <linearGradient
          id="paint1_linear_2784_28975"
          x1="26.8245"
          y1="24.2845"
          x2="51.8145"
          y2="41.6054"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0.5" stopColor="#B3A8FF" />
          <stop offset="1" stopColor="#71CCEB" />
        </linearGradient>
        <linearGradient
          id="paint2_linear_2784_28975"
          x1="27.9487"
          y1="27.2464"
          x2="44.7026"
          y2="40.9707"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0.5" stopColor="#B3A8FF" />
          <stop offset="1" stopColor="#71CCEB" />
        </linearGradient>
        <linearGradient
          id="paint3_linear_2784_28975"
          x1="12.4095"
          y1="27.6143"
          x2="32.7858"
          y2="44.306"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0.5" stopColor="#B3A8FF" />
          <stop offset="1" stopColor="#71CCEB" />
        </linearGradient>
        <linearGradient
          id="paint4_linear_2784_28975"
          x1="23.4693"
          y1="27.6543"
          x2="23.4693"
          y2="41.6717"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#005251" />
          <stop offset="1" stopColor="#005251" stopOpacity="0" />
        </linearGradient>
        <linearGradient
          id="paint5_linear_2784_28975"
          x1="29.8013"
          y1="27.3359"
          x2="29.8013"
          y2="42.3359"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#222CBB" />
          <stop offset="1" stopColor="#222CBB" stopOpacity="0" />
        </linearGradient>
        <linearGradient
          id="paint6_linear_2784_28975"
          x1="23.4695"
          y1="13"
          x2="23.4695"
          y2="25.106"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#005251" />
          <stop offset="1" stopColor="#005251" stopOpacity="0" />
        </linearGradient>
        <linearGradient
          id="paint7_linear_2784_28975"
          x1="24.1678"
          y1="12.3359"
          x2="24.1678"
          y2="25.3422"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#222CBB" />
          <stop offset="1" stopColor="#222CBB" stopOpacity="0" />
        </linearGradient>
        <clipPath id="clip0_2784_28975">
          <rect width="56" height="56" rx="28" fill="white" />
        </clipPath>
      </defs>
    </svg>
  );
};

export const RPDocument = () => {
  return (
    <svg
      width="56"
      height="56"
      viewBox="0 0 56 56"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g clipPath="url(#clip0_2784_29022)">
        <circle
          cx="27.598"
          cy="28"
          r="28"
          transform="rotate(-45 27.598 28)"
          fill="url(#paint0_linear_2784_29022)"
        />
        <mask
          id="mask0_2784_29022"
          style={{ maskType: "alpha" }}
          maskUnits="userSpaceOnUse"
          x="16"
          y="14"
          width="25"
          height="30"
        >
          <rect
            x="16.5"
            y="44"
            width="30"
            height="24"
            rx="2.8"
            transform="rotate(-90 16.5 44)"
            fill="#B3A8FF"
          />
        </mask>
        <g mask="url(#mask0_2784_29022)">
          <path
            d="M40.5 39L35 44H16.5V14H40.5V39Z"
            fill="url(#paint1_linear_2784_29022)"
          />
        </g>
        <path
          opacity="0.5"
          d="M35 44H18.5C31.3333 40.4375 37.4444 35.6875 40.5 25V39H36C35.4477 39 35 39.4477 35 40V44Z"
          fill="url(#paint2_linear_2784_29022)"
        />
        <path
          d="M21 21H36"
          stroke="#3E2FA6"
          strokeOpacity="0.5"
          strokeWidth="1.2"
          stroke-linecap="round"
        />
        <path
          d="M21 25H36"
          stroke="#3E2FA6"
          strokeOpacity="0.5"
          strokeWidth="1.2"
          stroke-linecap="round"
        />
        <path
          d="M21 29H36"
          stroke="#3E2FA6"
          strokeOpacity="0.5"
          strokeWidth="1.2"
          stroke-linecap="round"
        />
        <path
          d="M21 33H31"
          stroke="#3E2FA6"
          strokeOpacity="0.5"
          strokeWidth="1.2"
          stroke-linecap="round"
        />
      </g>
      <defs>
        <linearGradient
          id="paint0_linear_2784_29022"
          x1="27.598"
          y1="-1.90735e-06"
          x2="27.598"
          y2="56"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0.5" stopColor="#E7E3FF" />
          <stop offset="1" stopColor="#C5F5FC" />
        </linearGradient>
        <linearGradient
          id="paint1_linear_2784_29022"
          x1="16.9284"
          y1="13.9143"
          x2="47.631"
          y2="26.2102"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0.5" stopColor="#B3A8FF" />
          <stop offset="1" stopColor="#71CCEB" />
        </linearGradient>
        <linearGradient
          id="paint2_linear_2784_29022"
          x1="39.8889"
          y1="43.4062"
          x2="29.7993"
          y2="33.0217"
          gradientUnits="userSpaceOnUse"
        >
          <stop stopColor="#222CBB" />
          <stop offset="1" stopColor="#222CBB" stopOpacity="0" />
        </linearGradient>
        <clipPath id="clip0_2784_29022">
          <rect width="56" height="56" rx="28" fill="white" />
        </clipPath>
      </defs>
    </svg>
  );
};
