import { type SVGProps } from "react";

interface IProps extends SVGProps<SVGSVGElement> {}

export const Planning = (props: IProps) => {
  return (
    <svg
      width="90"
      height="90"
      viewBox="0 0 90 90"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g clip-path="url(#clip0_648_39921)">
        <g opacity="0.7">
          <path
            d="M65.4638 29.1657C64.2552 26.5737 61.1742 25.4523 58.5823 26.661L53.8892 28.8494L58.266 38.2356L62.9591 36.0472C65.5511 34.8386 66.6725 31.7576 65.4638 29.1657Z"
            fill="#D6D0FF"
          />
          <rect
            x="47.3262"
            y="14.7699"
            width="6.21394"
            height="41.4262"
            rx="3.10697"
            transform="rotate(-25 47.3262 14.7699)"
            fill="#7360FF"
          />
          <rect
            x="47.3262"
            y="14.7699"
            width="6.21394"
            height="41.4262"
            rx="3.10697"
            transform="rotate(-25 47.3262 14.7699)"
            fill="url(#paint0_linear_648_39921)"
          />
          <path
            d="M63.7368 49.9686L48.4393 51.0453C47.5755 51.1061 46.7195 51.2501 45.8835 51.4752L40.4412 52.9409L49.7118 66.6103C49.8711 66.8452 50.0774 67.0446 50.317 67.1966L53.2468 69.0544C54.629 69.9309 54.4967 71.9893 53.0135 72.6812L46.1516 75.881C45.302 76.2771 44.2919 76.0533 43.6893 75.3353L28.8608 57.661L28.3068 57.9194C23.5117 60.1553 17.8115 58.0807 15.5755 53.2856L14.9193 51.8783C12.6833 47.0832 14.758 41.3831 19.5531 39.1471L28.2347 35.0988L35.7877 29.8248C36.4976 29.329 37.1581 28.7659 37.7599 28.1433L48.4178 17.1167L63.7368 49.9686Z"
            fill="#7360FF"
          />
          <path
            d="M63.7368 49.9686L48.4393 51.0453C47.5755 51.1061 46.7195 51.2501 45.8835 51.4752L40.4412 52.9409L49.7118 66.6103C49.8711 66.8452 50.0774 67.0446 50.317 67.1966L53.2468 69.0544C54.629 69.9309 54.4967 71.9893 53.0135 72.6812L46.1516 75.881C45.302 76.2771 44.2919 76.0533 43.6893 75.3353L28.8608 57.661L28.3068 57.9194C23.5117 60.1553 17.8115 58.0807 15.5755 53.2856L14.9193 51.8783C12.6833 47.0832 14.758 41.3831 19.5531 39.1471L28.2347 35.0988L35.7877 29.8248C36.4976 29.329 37.1581 28.7659 37.7599 28.1433L48.4178 17.1167L63.7368 49.9686Z"
            fill="url(#paint1_linear_648_39921)"
          />
          <path
            d="M26.9599 54.6487L21.0005 57.9385L23.049 58.3377C24.2936 58.5803 25.5744 58.5694 26.8147 58.3056L28.7812 57.8874L36.8124 53.492L31.4566 42.0064L31.7902 45.4942C32.148 49.2356 30.2502 52.8323 26.9599 54.6487Z"
            fill="url(#paint2_linear_648_39921)"
            fill-opacity="0.5"
          />
          <path
            d="M45.4935 51.5151L40.1232 52.9235L48.8492 47.2285L50.2101 46.0603C52.9945 43.6703 54.4314 40.0679 54.0563 36.4177L53.0927 27.0391L63.8044 50.0105L47.5334 51.1774C46.8445 51.2268 46.1616 51.3399 45.4935 51.5151Z"
            fill="url(#paint3_linear_648_39921)"
            fill-opacity="0.5"
          />
          <path
            d="M33.6135 57.1348L29.4395 57.9059L40.3909 52.7991L48.88 65.4252L44.3187 60.8317C43.6836 60.1922 42.9697 59.6361 42.194 59.1771L40.9448 58.4377C38.7387 57.132 36.1344 56.6692 33.6135 57.1348Z"
            fill="url(#paint4_linear_648_39921)"
            fill-opacity="0.5"
          />
        </g>
      </g>
      <defs>
        <linearGradient
          id="paint0_linear_648_39921"
          x1="35.7828"
          y1="-0.289378"
          x2="53.5082"
          y2="56.186"
          gradientUnits="userSpaceOnUse"
        >
          <stop stop-color="#4FBCCF" />
          <stop offset="1" stop-color="#7360FF" />
        </linearGradient>
        <linearGradient
          id="paint1_linear_648_39921"
          x1="9.8491"
          y1="38.3539"
          x2="57.6081"
          y2="32.0995"
          gradientUnits="userSpaceOnUse"
        >
          <stop stop-color="#4FBCCF" />
          <stop offset="1" stop-color="#7360FF" />
        </linearGradient>
        <linearGradient
          id="paint2_linear_648_39921"
          x1="28.1062"
          y1="44.721"
          x2="33.4569"
          y2="56.1958"
          gradientUnits="userSpaceOnUse"
        >
          <stop stop-color="#222CBB" />
          <stop offset="1" stop-color="#222CBB" stop-opacity="0" />
        </linearGradient>
        <linearGradient
          id="paint3_linear_648_39921"
          x1="52.5282"
          y1="41.4879"
          x2="57.9978"
          y2="53.2176"
          gradientUnits="userSpaceOnUse"
        >
          <stop stop-color="#222CBB" />
          <stop offset="1" stop-color="#222CBB" stop-opacity="0" />
        </linearGradient>
        <linearGradient
          id="paint4_linear_648_39921"
          x1="39.5351"
          y1="54.2046"
          x2="45.7182"
          y2="67.4643"
          gradientUnits="userSpaceOnUse"
        >
          <stop stop-color="#222CBB" />
          <stop offset="1" stop-color="#222CBB" stop-opacity="0" />
        </linearGradient>
        <clipPath id="clip0_648_39921">
          <rect width="90" height="90" rx="45" fill="white" />
        </clipPath>
      </defs>
    </svg>
  );
};
