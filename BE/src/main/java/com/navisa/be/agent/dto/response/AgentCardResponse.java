...
public record AgentCardResponse(
    ...
    public static AgentCardResponse projectionToDto(AgentSimpleProjection agent, String profileImageUrl, List<Long> specialities, List<Long> badges, UserType requestUserType) {
        return new AgentCardResponse(
                agent.agentId(),
                agent.name(),
                profileImageUrl,
                ...
                badges,
                requestUserType.equals(UserType.FILLED_FOREIGNER)
                        ? (agent.specialityJobCount() != null ? agent.specialityJobCount().intValue() : 0 )
                        : null);
    }
}