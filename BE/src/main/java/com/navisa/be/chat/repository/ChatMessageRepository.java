...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, Long>, ChatMessageRepositoryQueryDsl {

    @Query(value = "SELECT * FROM chat_message cm " +
            "WHERE (cm.chat_room_id, cm.sent_at, cm.chat_message_id) IN (" +
            "  SELECT sub.chat_room_id, sub.sent_at, sub.chat_message_id FROM (" +
            "    SELECT chat_room_id, sent_at, chat_message_id, " +
            "           ROW_NUMBER() OVER (PARTITION BY chat_room_id ORDER BY sent_at DESC, chat_message_id DESC) as rn " +
            "    FROM chat_message " +
            "    WHERE chat_room_id IN :chatRoomIds" +
            "  ) sub WHERE sub.rn = 1" +
            ")", nativeQuery = true)
    List<ChatMessage> findAllLastChatMessageGroupByChatRoom(@Param("chatRoomIds") List<Long> chatRoomIds);

    @Query("SELECT cm.chatRoom.id AS id, COUNT(cm) AS count " +
            "FROM ChatMessage cm " +
