    ...
    public void publishRequestMessages() {
        ...
        for (ApplicationForm form : allRequiringEnd) {
            ...
            transactionTemplate.executeWithoutResult(status -> {
                try {
                    ChatRoom chatRoom = chatRoomQueryService.findByAgentProfileAndForeignerProfile(form.getAgentProfile(), form.getForeignerProfile());
                    // 피드백 요청 메세지 생성
                    ...
                    chatServiceFacade.saveAndPublishChatMessage(form.getAgentProfile().getUserId(), chatRequest, chatRoom);
