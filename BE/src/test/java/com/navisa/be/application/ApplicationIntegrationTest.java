...
class ApplicationIntegrationTest extends IntegrationTestSupport {
    ...
    void visaProcess_FullCycle_Integration() {
        ...
        VisaApplicationForm form = setupInitialForm(agentUser);

        // 수임 종료 API 호출
        VisaApplicationFinishResponse response = applicationCommandService.finishApplication(agentUser.getEmail(), form.getId(), true);

        // 데이터베이스 최종 상태 검증
        List<VisaApplicationForm> allForms = applicationFormRepository.findAll();

        // 기존 폼 1개 + 신규 폼 1개 = 총 2개
        assertThat(allForms).hasSize(2);

        // 기존 폼 상태 확인: isFinished = true
        VisaApplicationForm updatedOldForm = applicationFormRepository.findById(form.getId()).get();
        assertThat(updatedOldForm.isFinished()).isTrue();

        // 신규 폼 상태 확인: 행정사 정보가 없고 새롭게 시작 가능한 상태
        VisaApplicationForm nextCycle = applicationFormRepository.findById(response.newVisaFormId()).get();
    ...
    private VisaApplicationForm setupInitialForm(User agentUser) {
        ...
        VisaApplicationForm form = new VisaApplicationForm(agent, foreigner, jobCode, false, 150, 1);
