...
class ChatMessageRepositoryTest extends IntegrationTestSupport {
    ...
    void findAllLastChatMessageGroupByChatRoom() {
        ...
        ChatRoom chatRoom2 = chatRoomTestFixture.createChatRoom(foreignerProfile2, agentProfile, ChatRoomStatus.DEFAULT);
        ...
        // when
        List<LastMessageProjection> lastMessages = chatMessageRepository
    ...
    void findCountByChatRoomIn() {
        ...
        ChatMessageNonReadCountProjection countProj = result.stream()
                .filter(p -> p.id().equals(chatRoom1.getId()))
                .findFirst()
                .orElseThrow();

        assertThat(countProj.count()).isEqualTo(2L);
    }
    ...
    void findChatMessagesByChatRoomIdAndNoOffset() {
        ...
        ChatRoom targetRoom = chatRoomTestFixture.createChatRoom(foreignerProfile, agentProfile1, ChatRoomStatus.DEFAULT);
        ChatRoom otherRoom = chatRoomTestFixture.createChatRoom(foreignerProfile, agentProfile2, ChatRoomStatus.DEFAULT);

        // 타겟 채팅방 메시지 30개 생성
        for (int i = 1; i <= 30; i++) {
            ChatMessage message = chatRoomTestFixture.createChatMessage(targetRoom,
                    foreignerProfile.getId(), "Target " + i, false);
        ...
        // 소음용 메시지 10개 생성
        for (int i = 1; i <= 10; i++) {
            ChatMessage otherMessage = chatRoomTestFixture.createChatMessage(otherRoom,
                    foreignerProfile.getId(), "Other " + i, false);
        ...
        List<ChatMessage> page1 = chatMessageRepository
                .findChatMessagesByChatRoomIdAndNoOffset(targetRoom.getId(), sliceRequest1);
        ...
        List<ChatMessage> page2 = chatMessageRepository
                .findChatMessagesByChatRoomIdAndNoOffset(targetRoom.getId(), sliceRequest2);
